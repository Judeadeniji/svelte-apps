...
<a href={url} class={classes}>
  <div class="flex flex-row gap-3 w-full border border-coal-200
    dark:border-turpentine-300 p-4 rounded-3xl">
    <h2 class="font-semibold flex-1 text-2xl lg:text-3xl hover:underline leading-snug mb-4 dark:text-white">
      {title}
    </h2>
  </div>
  <div>
    <div class="flex flex-row justify-start gap-2 items-center text-base
    text-tertiary dark:text-tertiary-dark font-bold mt-3 mr-4">
      {#if icon === 'labs'}
      <svg
      ...
      <span>{date}</span> 
      {#if badge}
      <div class="h-fit px-1 bg-highlight dark:bg-highlight-dark rounded uppercase text-link dark:text-link-dark font-bold tracking-wide text-xs whitespace-nowrap">
        New
      </div>
      {/if}
    </div>
    <span class="text-base text-secondary dark:text-secondary-dark">
      <slot></slot>
    </span>
    {#if  null}
    <div class="text-link text-base dark:text-link-dark hover:underline mt-4">
      Read more
    </div>
    {/if}
  </div>
